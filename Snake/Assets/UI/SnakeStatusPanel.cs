...
namespace Snake.UI
{
    ...
    /// </summary>
    public class SnakeStatusPanel : TerminalWindow
    {
        ...
        public void Bind(SnakeMatchManager match, SnakeProgram playerProgram)
        {
            _match = match;
        ...
        private string[] BuildMatchSection(int maxRows)
        {
            ...
            if (_match != null)
            {
                ...
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightGreen, $"{_match.HighScore}")} {emdash} HIGH");

                int length = _match.Grid != null ? _match.Grid.Body.Count : 0;
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightYellow, $"{length}")} {emdash} LENGTH");

                int eaten = _match.Grid != null ? _match.Grid.FoodEaten : 0;
                lines.Add($"  {TUIColors.Fg(TUIColors.BrightMagenta, $"{eaten}")} {emdash} FOOD EATEN");

                string dir = _match.Grid != null
                    ? _match.Grid.CurrentDirection.ToString().ToUpper()
                    : "?";
                lines.Add($"  {dir} {emdash} DIRECTION");

                float interval = _match.CurrentStepInterval;
                lines.Add($"  {TUIColors.Dimmed($"{interval:F3}s")} {emdash} STEP RATE");

                lines.Add($"  {_match.MatchesPlayed} {emdash} MATCHES");
